<template>
    <div class="order">
        <div class="order__title">
            <h3>Быстрий заказ</h3>
            <div class="my-1">
                <div class="alert alert-success" v-if="isOrdered || successCreated">
                    <strong>Вы уже делали заказ на этот товар.</strong>
                </div>
                <!-- /.alert alert-success -->
            </div>
            <!-- /.my-1 -->
            <button v-if="!showOrderForm" class="btn btn-success btn-lg" @click="showForm">
        ...
        <div class="order__form" v-show="showOrderForm">
            ...
            <div class="form-group">
                <vSelect :options="sizes" v-model="selectSize" placeholder="Какой Ваш розмер?" > </vSelect>
            </div>
            <!-- /.form-group -->
            <div class="form-group">
            ...
            <!-- /.form-group -->
            <div class="form-group">
                <div class="d-flex align-items-baseline" role="group" aria-label="Использовать месенжер">
                    <input 
                    type="checkbox" 
                    class="btn-check" 
                    id="use-messager" 
                    ...
                    for="use-messager"
                    >Использовать месенжеры <info>Поставьте галочку что-бы мы общались с вами только через месенджеры, такие как Telegram, Viber, WatsApp</info>
                    </label>
                </div>
            </div>
            <!-- /.form-group -->
...
export default {
    ...
    methods:{
        ...
        doOrder(){
            ...
            route('new.order')
            .then(r => {
                axios.post(r.data, this.formData)
                .then(resp => {
