...
<template>
  <div class="imageItem">
    <!-- Close Icob -->
        <svg v-if="deleteIcon" @click="doDelete" xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="red" class="closeSvg bi bi-x-circle-fill" viewBox="0 0 16 16">
            <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0zM5.354 4.646a.5.5 0 1 0-.708.708L7.293 8l-2.647 2.646a.5.5 0 0 0 .708.708L8 8.707l2.646 2.647a.5.5 0 0 0 .708-.708L8.707 8l2.647-2.646a.5.5 0 0 0-.708-.708L8 7.293 5.354 4.646z"/>
        </svg>
    <label >
      <input type="file" ref="file" @change="doSubmit" />
      <span>
        <div v-if="spiner" class="spinner-border" role="status">
            <span class="hidden visually-hidden">Loading...</span>
        </div>
        <img v-if="imageBox" :src="imageSrc" alt="">
        <strong v-if="errorText.length > 0">{{errorText}}</strong>
        <!-- LoadIcon -->
        <svg v-if="icon" xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="currentColor" class="bi bi-file-earmark-plus-fill" viewBox="0 0 16 16">
...
export default {
  ...
  computed:{
      imageSrc(){
          if (typeof this.responseFile === 'object' && this.responseFile.uri !== undefined) {
              return this.responseFile.uri;
          }
          return '';
      },
      isWebP(){
          if (typeof this.responseFile === 'object' && this.responseFile.type !== undefined) {
